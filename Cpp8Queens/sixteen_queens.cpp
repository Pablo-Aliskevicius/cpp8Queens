...
namespace qns16
{
    ...
    static constexpr size_t maximum_allowed_board_size = 16; // memory allocations are based on this. 
    ...
    // We shall only show the first 50
    static const int max_solutions_to_show = 50;
    static std::vector<std::vector<int>> solutions(max_solutions_to_show, std::vector<int>(16, -1));
...
{.m256i_u16 = {0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x8000, } },
    ...
    const std::vector<int>& not_threatened_rows(const map_t map, int current_column)
    {
        ...
        if (!j)
        {
            ...
            using std::endl;
            cout 
                << "All rows are threatened, shouldn't have called this function (current column is" 
                << current_column << ") " << endl 
                << "Threats:" << endl;
            for (int i = 0; i < board_size; ++i)
            {
                cout << hex << mask.m256i_u16[i] << " ";
                cout << endl;
            }
            cout << endl << "The current map is:" << endl;
            for (int i = 0; i < board_size; ++i)
            {
                cout << hex << map.m256i_u16[i] << " ";
                cout << endl;
            }
            throw std::exception("All rows are threatened, shouldn't have called this function");
        }
        if (j < result.size())
        {
            result[j] = sentinel;
    ...
    public:
        ...
        map_t Get(int row, int col) const  // hopefully inline
        { 
            return m_threats[(size_t)row * maximum_allowed_board_size + col]; 
        }
    }; 
    ...
    {
        // Equivalent to this:
        // map |= make_threat(row, column)
        // Which is equivalent to this:
        // map |= (row_masks[row] | main_diagonal_parallels[row + 15 - column] | second_diagonal_parallels[row + column]);
    ...
    void do_solve(map_t map, std::vector<int>& solution, int current_column)
    {
        if (current_column == (board_size - 1))
        {
            ...
            if (success_count < solutions.size())
            {
                ...
                // INVARIANT: The destination has 16 integers, and the source - board_size.
